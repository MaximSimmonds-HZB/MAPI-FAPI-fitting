...
def DBTD_multi_trap(parameters, t, Gpulse, t_span, N0=0, G_frac = 1, equilibrate=True, eq_limit=1e-2, maxcount=1e3, output_integrated_values = True,**kwargs):
    ...
    ## M.S Tool for comparison with own functions, to delete
    # k_direct = 1.969782507644605e-17
    # Eg = 1.553
    # L = 450e-9
    # alpha = 6434830.8863374945
    # N_c = N_v = 2e24
    # E_t_bulk_list = np.array([0.2000274614703775, 1.3393846112411676])
    # N_t_bulk_list = np.array([1.1855005474647105e+23, 1.3651312249108357e+21])
    # C_n_bulk_list = np.array([4.241814891012085e-15, 1.7259123921493207e-13])
    # C_p_bulk_list = np.array([8.851864138905135e-19, 1.1372542744335435e-16])
    # T = 300
    ...
    N_init = 1.3934546562919416e+20#N0 *G_frac# initial conditions
    ...
    def jacobian_no_flux_vectorized_fixed(t, P_flat, *args):
        ...
        # Diffusion for n
        ## M.S Correction: factor 2 in front of the diffusion, corresponding to the second derivative correction. 
    ...
    if dimentionless:
        ...
        ## !! Important !! M.S Correction, dx should be the last argument, i.e. non dimensionalised length (otherwise it doesnt solve)
    ...
    t_start = time.time()
    count = 0
    ## M.S Proposition: BDF is the most stable version and should be used all the time. also enables the use of the jacobian, making the solver run much faster.
    #--> method = kwargs.get('method', 'BDF')  # default method for solve_ivp ??
    method = kwargs.get('method', 'Radau')  # default method for solve_iv
    rtol = kwargs.get('rtol', 1e-3)
    ...
    if equilibrate:
        ...
        #     print(f"Equilibrating {count} times ",parameters)+
            ...
            if dimentionless:
                ...
                #solve_ivp(self.model_vect_dimensionless, t_span, P0, method='BDF', args=arg, vectorized=True, jac=self.jacobian_no_flux_vectorized_fixed, rtol=1e-3, atol=1e-3)
                ## M.S Correction, for now BDF is hard coded
                sol_single = solve_ivp(model_vect_dimensionless, [t_span[0],t_span[-1]], P0, method='BDF', args=arg, vectorized=True,  rtol=rtol, atol=atol,t_eval=t_span, jac=jacobian_no_flux_vectorized_fixed,)
            else:
                ## M.S Correction, for now BDF is hard coded
                sol_single = solve_ivp(model_vect, [t_span[0],t_span[-1]], P0, method='BDF', args=arg, vectorized=True,  rtol=rtol, atol=atol,t_eval=t_span,jac=jacobian_no_flux_vectorized_fixed,)
    ...
    if dimentionless:
        t = t/ tau
        ## M.S Correction, for now BDF is hard coded
        sol = solve_ivp(model_vect_dimensionless, [t_span[0],t_span[-1]], P0, method='BDF', args=arg, vectorized=True,  rtol=rtol, atol=atol,t_eval=t, jac=jacobian_no_flux_vectorized_fixed,)
    else:
        ## M.S Correction, for now BDF is hard coded
        sol = solve_ivp(model_vect, [t_span[0],t_span[-1]], P0, method='BDF', args=arg, vectorized=True,  rtol=rtol, atol=atol,t_eval=t,jac=jacobian_no_flux_vectorized_fixed,)#,jac=jacobian_no_flux_vectorized_fixed)#,jac=jacobian_no_flux_vectorized_fixed)
    if not(sol.success):
        logger.warning("ODE solver did not converge, returning NaN arrays.")
        return np.nan * np.ones((len(t),grid_size)), np.nan * np.ones((len(t),grid_size))
    ...
    ## M.S Tool for comparison with own functions, to delete
    #     y_single = k_direct*np.trapezoid(ni**2*sol_flat[0, :, :]*sol_flat[1, :, :], axis = 0, x = z_array)
    #     y_single_normalised = y_single/np.amax(y_single) + 0.000979955456570156
    # else:
    #     y_single = k_direct*np.trapezoid(sol_flat[0, :, :]*sol_flat[1, :, :], axis = 0, x = z_array)
    #     y_single_normalised = y_single/np.amax(y_single) + 0.000979955456570156
    ...
    if output_integrated_values:
        ...
        #return np.log10(1e6*(y_single_normalised))
    else:
